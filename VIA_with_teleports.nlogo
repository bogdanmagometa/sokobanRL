...
to go
  ...
  ask agens with [who != [who] of my-turtle] [
    let best-action 0
    ask my-turtle [
      ...
      foreach (list 90 180 270 0) [
        ...
        ;  [a] ->
          ...
          ask my-turtle [
            ...
            let current-utility get-utility x y cur_heading
            table:put new-values (list x y cur_heading) best-utility
...
to-report get-reward-plus-utility-for-deteministic-action [a]
  ...
  let new-utility get-utility xcor ycor heading
...
to put-utility [x y headin utility]
  let state (list x y headin)
  table:put u state utility
end

to-report get-utility [x y headin]
  let state (list x y headin)
  if (table:has-key? u state) [
    report table:get u state
  ]
  put-utility x y headin 0
  report table:get u state
end
